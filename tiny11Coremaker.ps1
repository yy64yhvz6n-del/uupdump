...
if (-not $UserProvidedAutounattend) {
    Write-Host "Downloading default Tiny11 autounattend.xml..."
...
if (Test-Path "$mainOSDrive\tiny11\sources\install.esd") {
    Set-ItemProperty -Path "$mainOSDrive\tiny11\sources\install.esd" -Name IsReadOnly -Value $false
    Remove-Item "$mainOSDrive\tiny11\sources\install.esd"
}
...
if ($StripShortNames) {
    Write-Host "Disabling 8.3 name creation permanently in the OS:"
    & 'reg' 'add' 'HKLM\zSYSTEM\ControlSet001\Control\FileSystem' '/v' 'NtfsDisable8dot3NameCreation' '/t' 'REG_DWORD' '/d' '1' '/f' | Out-Null
}
...
Write-Host "Copying custom ISOFILES..."
...
# Stop the transcript
if (-not $UserProvidedAutounattend) {
    Write-Host "Removing downloaded autounattend.xml..."
